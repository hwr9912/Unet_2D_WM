...
class MyDataset(data.Dataset):
    def __init__(self, root, resize_h=512, resize_w=512):

        imgs = []
        img_list = os.listdir(os.path.join(root, "images"))
        img_list = sorted(img_list)
        label_list = os.listdir(os.path.join(root, "masks"))
        label_list = sorted(label_list)

        for i in range(len(img_list)):
            img = img_list[i]
            mask = label_list[i]
            imgs.append(["{}/images/".format(root) + img, "{}/masks/".format(root) + mask])
